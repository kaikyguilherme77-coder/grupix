export default async function handler(req, res) {
  ...
  if (!groupName || !email || !link || !planType) {
  ...
  const PLANOS = {
    basico:   { dias: 7,  preco: 10.00, label: 'Destaque 7 dias'  },
    vip:      { dias: 15, preco: 20.00, label: 'Destaque 15 dias' },
    boost:    { dias: 30, preco: 30.00, label: 'Destaque 30 dias' },
    semana:   { dias: 7,  preco: 10.00, label: 'Destaque 7 dias'  },
    quinzena: { dias: 15, preco: 20.00, label: 'Destaque 15 dias' },
    mensal:   { dias: 30, preco: 30.00, label: 'Destaque 30 dias' },
  }
  ...
  try {
    ...
    if (!data.init_point) {
      ...
      return res.status(500).json({ error: 'Erro ao criar pagamento', detail: data.message || data })
    }
